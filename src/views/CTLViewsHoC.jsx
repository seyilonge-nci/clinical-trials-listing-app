...
const CTLViewsHoC = (WrappedView) => (props) => {
	...
	const getFetchByIdOrName = paramType === 'code' ? getListingInformationById : getListingInformationByName;
	const getListingInfo = useCustomQuery(getFetchByIdOrName({ queryParam }), shouldFetchListingInfo);
		...
		<div>
			{(() => {
				if (!doneLoading) {
					...
					return <WrappedView {...props} listingInfo={stateListingInfo} />;
				}
			})()}
		</div>
	);
